...
\begin{definition}
    A non-zero vector $v \in V$ is called an \cindex{eigenvector} of linear operator $T$ if $\exists \lambda$ that $T(v) = \lambda v$. $\lambda$ is called \cindex{eigenvalue} corresponding to eigenvector $v$. So it is for matrix. A eigenvalue could be $0$, but eigenvector could not be $\vec{0}$.
\end{definition}
...
\begin{definition}
    Let $W_i$ be subspaces of a vector space $V$. The \cindex{sum} of these subspaces is defined as $\displaystyle \sum_{i=1}^k W_i = \{ v_1 + v_2 + \dots + v_k : v_i \in W_i \text{ for } 1 \leq i \leq k \}$
\end{definition}
