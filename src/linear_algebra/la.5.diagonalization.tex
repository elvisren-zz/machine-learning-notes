...
\begin{definition}
    A \emph{non-zero} vector $v \in V$ is called an \cindex{eigenvector} of linear operator $T$ if $\exists \lambda : T(v) = \lambda v$. $\lambda$ is called \cindex{eigenvalue} corresponding to eigenvector $v$. So it is for matrix. A eigenvalue could be $0$, but eigenvector could not be $\vec{0}$. Eigenvector will for an invariant subspace of dimension $1$.
\end{definition}
...
\begin{definition}
    For $A = [T]_\beta$ the polynomial $f_A(t) = \determinate{A - t I_n}$ is called the \cindex{characteristic polynomial} of $A$ and $T$. 
\end{definition}
