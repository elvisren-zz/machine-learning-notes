...
\begin{definition}
	...
	$(-1)^{i + j} A_{ij} \times \determinate{\tilde{A}_{ij}}$ is called the \cindex{cofactor} of $A$ in row $i$ and column $j$.
...
\begin{theorem}
	In geometry, for a square matrix $A \in M_{n\times n}(F)$, $\absolutevalue{\text{det}(A)}$ is the \cindex{n-dimensional volume} of the parallelepiped having vector $a_i$ as adjacent sides.
\end{theorem}
